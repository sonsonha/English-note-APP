...
const WordDetail = ({ wordId, setScreen }) => {
  ...
  const vi = window.getVietnamese ? window.getVietnamese(w?.text) : null;
    ...
    <div className="canvas fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 28 }}>
        ...
        <div>
            ...
            onKeyDown={(e) => e.key === " " && (e.preventDefault(), setFlipped(!flipped))}>
              ...
              <div className="flip-face flip-back">
                <div className="row between">
                  <div className="kicker" style={{ color: "oklch(1 0 0 / 0.85)" }}>Your meaning</div>
                  <button className="btn btn-ghost btn-icon" onClick={(e) => {e.stopPropagation();setEditingMeaning((v) => !v);}}
                  style={{ color: "oklch(1 0 0 / 0.85)" }} title="Edit">
                ...
                <textarea
                  ...
                  rows={2}
                  style={{
                    marginTop: 8, width: "100%", background: "oklch(1 0 0 / 0.18)",
                    border: "1.5px solid oklch(1 0 0 / 0.30)", borderRadius: 8, padding: "10px 12px",
                ...
                <div className="row faint" style={{ fontSize: 12.5, marginTop: "auto", justifyContent: "space-between", color: "oklch(1 0 0 / 0.78)" }}>
                  <div className="row" style={{ gap: 8 }}>
          ...
          {/* Vietnamese meaning — click to reveal */}
          <div className="card" style={{ marginTop: 18, padding: 0, overflow: "hidden", borderColor: showVi ? "var(--accent)" : "var(--paper-edge)" }}>
            <button onClick={() => setShowVi(v => !v)}
              ...
              <span style={{ transform: showVi ? "rotate(180deg)" : "none", transition: "transform .2s", color: "var(--ink-mute)" }}>
            ...
            {showVi && (
              <div style={{ padding: "18px 22px 22px", borderTop: "1.5px dashed var(--paper-edge)", background: "var(--paper)" }}>
                {vi ? (
                  <>
                    <div style={{ fontFamily: "var(--display)", fontWeight: 700, fontSize: 24, letterSpacing: "-0.015em", color: "var(--ink)" }}>
                      {vi.meaning}
                    </div>
                    <div className="card-soft" style={{ marginTop: 14, padding: "14px 16px", background: "var(--accent-tint)", border: "none", borderRadius: 12 }}>
                      <div className="kicker" style={{ marginBottom: 4, color: "var(--accent-deep)" }}>Example</div>
                      <div style={{ fontSize: 15.5, lineHeight: 1.5, color: "var(--ink-2)", fontStyle: "italic" }}>
                        "{vi.example}"
                      </div>
                    </div>
                  </>
                  ...
                  <div className="muted" style={{ fontSize: 14 }}>
                    No Vietnamese translation yet for this word. <a href="#" onClick={(e) => e.preventDefault()}>Contribute one</a>.
                  </div>
                )}
              </div>
            )}
