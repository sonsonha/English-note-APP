...
const Customizer = ({ max, onBegin, onCancel, accent }) => {
  ...
  return (
    <div style={{padding:14, background: accent ? "oklch(0.30 0.02 60)" : "var(--paper-2)", borderRadius:10, border: accent ? "1px solid oklch(0.42 0.02 60)" : "1px solid var(--rule)"}}>
      <div className="kicker" style={{marginBottom:8, color: accent ? "oklch(0.78 0.018 70)" : undefined}}>Customize</div>
      ...
      {mode !== "all" && (
        <div className="row" style={{gap:6, marginBottom:10, flexWrap:"wrap"}}>
          ...
          <span className="faint" style={{fontSize:12, alignSelf:"center", color: accent ? "oklch(0.78 0.018 70)" : undefined}}>of {max}</span>
        </div>
      )}
...
const Calendar = ({ setScreen, openWord, startReview, goToLibrary }) => {
  ...
  return (
    <div className="canvas fade-in">
      ...
      {/* ─── Month summary ─── */}
      <SummaryCard
        title={monthLabel} days={monthDays} kind="month"
        scope={{ kind:"month", value: focusMonth.toISOString().slice(0,10) }}
        startReview={startReview} goToLibrary={goToLibrary}
      />

      {/* ─── Week summaries within this month ─── */}
      <div style={{marginTop:24}}>
        <div className="kicker" style={{marginBottom:10}}>Weeks in {monthLabel}</div>
        <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", gap:14}}>
          {weekStartsInMonth.map((w, i) => {
            const wdays = window.DAYS.filter(d => d.date >= w.start && d.date <= w.end);
            const info = getISOWeek(w.start);
            const total = isoWeeksInYear(info.year);
            return (
              <SummaryCard key={i}
                title={`Week ${info.week} of ${total}`}
                days={wdays}
                kind="week"
                scope={{ kind:"week", value: w.start.toISOString().slice(0,10) }}
                startReview={startReview} goToLibrary={goToLibrary} />
            );
          })}
        </div>
  ...
  <div className="page-header">
    <div>
      ...
      <div className="meta" style={{marginTop:6, maxWidth:"60ch"}}>
        Every dot is a day. Every label is a story — captured, tended, and (sometimes) mastered.
      </div>
    </div>
...
const WeekView = ({ title, sub, days, scope, onPrev, onNext, canNext, openWord, startReview, goToLibrary }) => {
  ...
  return (
    <div>
      ...
      {/* Week summary below */}
      <SummaryCard title={title} days={days} kind="week" scope={scope}
        startReview={startReview} goToLibrary={goToLibrary} />
    </div>
  );
};
...
const QuarterView = ({ title, sub, days, startM, year, scope, onPrev, onNext, canNext, openWord, startReview, goToLibrary }) => {
  ...
  return (
    <div>
      ...
      {/* Quarter summary below */}
      <SummaryCard title={title} days={days} kind="quarter" scope={scope}
        startReview={startReview} goToLibrary={goToLibrary} />
    </div>
  );
};
