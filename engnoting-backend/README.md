...
```http
GET /api/v1/reviews/session/current?session_id={session_id}
```
...
Records the review result for the current session item, updates statistics, and advances the session. The submitted `word_id` and `review_type` must match the current item for `session_id`.
...
#### Skip Current Session Item

```http
POST /api/v1/reviews/session/advance
...
Skips the current item without recording a review.
